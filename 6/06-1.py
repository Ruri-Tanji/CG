...
im = Image.open(os.path.dirname(__file__)+'INIAD_image.png')
g_theta = math.pi/2
g_phi = 0
texID = 0
...
def load_texture():
    ...
    for i in range(w):
        for j in range(h):
            data[1-j][i]= rgb_im.getpixel((i,j))## Store the pixel value of rgb_im[][] to data[][] ##
...
def draw_texture_cubic(x, y, z, length):
    ...
    # right (mapping INIAD red striped logo)
