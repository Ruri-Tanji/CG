...
light_array=np.array([[1.0,4.0,0.0], [0.8, 4.0, 0.0],[0.6, 4.0, 0.0],[1.2, 4.0, 0.0],[1.4, 4.0, 0.0],
            [1.0,4.0,0.2], [0.8, 4.0, 0.2],[0.6, 4.0, 0.2],[1.2, 4.0, 0.2],[1.4, 4.0, 0.2],
            [1.0,4.0,0.4], [0.8, 4.0, 0.4],[0.6, 4.0, 0.4],[1.2, 4.0, 0.4],[1.4, 4.0, 0.4],
            [1.0,4.0,-0.2], [0.8, 4.0, -0.2],[0.6, 4.0, -0.2],[1.2, 4.0, -0.2],[1.4, 4.0, -0.2],
            [1.0,4.0,-0.4], [0.8, 4.0, -0.4],[0.6, 4.0, -0.4],[1.2, 4.0, -0.4],[1.4, 4.0, -0.4]])
...
def draw_sphere_shadow(N, M):
    for j in range(0, N):
        ...
        for i in range(0, M):
            ...
            normal = np.array([math.sin(theta)*math.sin(phi), math.cos(theta), math.sin(theta)*math.cos(phi)])

            if np.dot(normal, p00 - light_pos) > 0:
                for k in range(len(light_array)):
                        sp_c = calculate_intersection_to_plane(np.array([0,2,0]), light_array[0], 0.0)
                        glColor4f(0.1, 0.1, 0.1, 1.0/10)
                        glBegin(GL_QUADS)
                        sp00 = calculate_intersection_to_plane(p00, light_array[k], 0.0) ## calculate intersection to xz plane using calculate_intersection_to_plane ##
                        glColor4f(0.1, 0.1, 0.1, 0.1*math.exp(-0.1*np.dot(sp_c-sp00, sp_c-sp00)**2))
                        glVertex3f(sp00[0], sp00[1], sp00[2])
                        sp01 = calculate_intersection_to_plane(p01, light_array[k], 0.0) ## calculate intersection to xz plane using calculate_intersection_to_plane ##
                        glColor4f(0.1, 0.1, 0.1, 0.1*math.exp(-0.1*np.dot(sp_c-sp01, sp_c-sp01)**2))
                        glVertex3f(sp01[0], sp01[1], sp01[2])
                        sp11 = calculate_intersection_to_plane(p11, light_array[k], 0.0) ## calculate intersection to xz plane using calculate_intersection_to_plane ##
                        glColor4f(0.1, 0.1, 0.1, 0.1*math.exp(-0.1*np.dot(sp_c-sp11, sp_c-sp11)**2))
                        glVertex3f(sp11[0], sp11[1], sp11[2])
                        sp10 = calculate_intersection_to_plane(p10, light_array[k], 0.0) ## calculate intersection to xz plane using calculate_intersection_to_plane ##
                        glColor4f(0.1, 0.1, 0.1, 0.1*math.exp(-0.1*np.dot(sp_c-sp10, sp_c-sp10)**2))
...
def idle():
    ...
    light_array[0] = np.array([2*math.sin(3*Lissa_t)+1.0, 4.0, 2*math.sin(4*Lissa_t)])
    glutPostRedisplay()
